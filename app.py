...
# ── Admin Credentials ──
ADMIN_USERNAME = "Damodhar Krishna"
...
def reset_password():
    if request.method == "POST":
        ...
        users[email]["last_password_reset"] = \
            get_ist_time()
        save_users(users)
...
def delete_chat(chat_id):
    user_id = session["user_id"]
    chats = load_chats()
    if user_id in chats and \
       chat_id in chats[user_id]:
